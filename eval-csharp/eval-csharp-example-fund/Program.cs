...
namespace eval_csharp_example_fund
{
    ...
    class Program
    {
        ...
        private async Task<String> latestRankAsync() {
            ...
            //结果要cache起来，注意cache的key为trade_date
            //
            ...
            //TODO high- 有可能其他线程/进程在查询期间把数据添加进去了，这时候我应该可以继续运行业务逻辑而不进行更新了 - 如何做？How？
            ...
            using (FundRecommandContext context = new FundRecommandContext())
            {
                ...
                await context.AddRankRawCacheAsync(tradeDate, rankStrategyId, rankSource, content);
            }
            return content;
        }

        //TODO 这个trade date当然不对啦，改一下
